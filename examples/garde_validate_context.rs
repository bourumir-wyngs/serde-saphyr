...
fn main() {
   ...
   "#;
    ...
    let context = ValidationContext { allowed_string: "test".to_string() };

    let err =
        serde_saphyr::from_str_with_options_context_valid::<AB>(yaml, Default::default(), &context).expect_err("must fail validation");
