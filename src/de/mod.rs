...
pub(crate) struct Cfg {
    ...
    /// If true, treat YAML merge keys (`<<`) as ordinary mapping keys.
    pub(crate) no_merge_keys: bool,
...
impl Cfg {
    ...
    pub(crate) fn from_options(options: &Options) -> Self {
        Self {
            ...
            no_merge_keys: options.no_merge_keys,
...
fn pending_entries_from_events<'a>(
    ...
    match replay.peek()? {
        ...
        Some(Ev::MapStart { .. }) => {
            collect_entries_from_map(&mut replay, reference_location, no_merge_keys)
        }
        Some(Ev::SeqStart { .. }) => {
    ...
    loop {
        match ev.peek()? {
            ...
            Some(_) => {
                ...
                if !no_merge_keys && is_merge_key(&key) {
                ...
                loop {
                    ...
                    match self.ev.peek()? {
                        ...
                        Some(_) => {
                            ...
                            if !self.cfg.no_merge_keys && is_merge_key(&key_node) {
...
mod tests {
    ...
    fn pending_from_events(
        ...
        pending_entries_from_events(events, location, reference_location, false)
    }
    ...
    fn pending_from_events(
        ...
        pending_entries_from_events(events, location, reference_location, false, None)
    }
    ...
    fn pending_entries_from_live_events_handles_null_scalars_sequences_and_eof() {
        ...
        assert!(
            pending_entries_from_live_events(&mut null_replay, merge_reference, false)
                .unwrap()
                .is_empty()
        );
        ...
        let entries =
            pending_entries_from_live_events(&mut seq_replay, merge_reference, false).unwrap();
    ...
    fn collect_entries_from_map_expands_merges_and_preserves_reference_locations() {
        ...
        let err = unwrap_err(collect_entries_from_map(&mut not_a_map, loc(34, 9), false));
        assert!(matches!(
            err,
        ...
        let entries = collect_entries_from_map(&mut replay, outer_reference, false).unwrap();
        assert_eq!(entries.len(), 2);
        assert_eq!(
            pending_pair(&entries[0]),
