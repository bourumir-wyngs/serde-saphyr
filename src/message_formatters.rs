...
///
/// This formatter sometimes produces recommendations on how to adjust settings and API
/// calls to make parsing work, so it normally shouldn't be user-facing. Use
/// `UserMessageFormatter` for user-facing content, or implement a custom
/// `MessageFormatter` for full control over output.
...
impl MessageFormatter for DefaultMessageFormatter {
    fn format_message<'a>(&self, err: &'a Error) -> Cow<'a, str> {
        Self::format_impl(self.localizer(), err)
    }
}
...
impl MessageFormatter for DefaultMessageFormatterWithLocalizer<'_> {
    ...
    fn format_message<'a>(&self, err: &'a Error) -> Cow<'a, str> {
        DefaultMessageFormatter::format_impl(self.localizer, err)
    }
}
...
impl DefaultMessageFormatter {
    ...
    fn format_impl<'a>(localizer: &dyn Localizer, err: &'a Error) -> Cow<'a, str> {
        match err {
            Error::WithSnippet { error, .. } => Self::format_impl(localizer, error),
            Error::ExternalMessage {
                ...
                localizer.override_external_message(ExternalMessage {
                    source: *source,
                    original: msg.as_str(),
                    code: code.as_deref(),
                    params,
            ...
            Error::AliasError { msg, locations } => {
                ...
                match (ref_loc, def_loc) {
                    ...
                    (_r, d) => Cow::Owned(format!("{msg}{}", localizer.alias_defined_at(d))),
                }
            }
            ...
            Error::ValidationError { report, locations } => {
                ...
                for issue in issues {
                    let entry = issue.display_entry_overridden(localizer, ExternalMessageSource::Garde);
                    let path_key = issue.path;
                    let original_leaf = path_key
                        .leaf_string()
                        .unwrap_or_else(|| localizer.root_path_label().into_owned());
                    ...
                    lines.push(localizer.validation_issue_line(
                        &resolved_path,
                        &entry,
                        (loc != Location::UNKNOWN).then_some(loc),
                    ));
                }
                Cow::Owned(localizer.join_validation_issues(&lines))
            }
            #[cfg(feature = "garde")]
            ...
            Error::ValidatorError { errors, locations } => {
                ...
                let mut lines = Vec::with_capacity(issues.len());
                for issue in issues {
                    let entry = issue.display_entry_overridden(localizer, ExternalMessageSource::Validator);
                    let path_key = issue.path;
                    let original_leaf = path_key
                        .leaf_string()
                        .unwrap_or_else(|| localizer.root_path_label().into_owned());
...
impl MessageFormatter for UserMessageFormatterWithLocalizer<'_> {
    ...
    fn format_message<'a>(&self, err: &'a Error) -> Cow<'a, str> {
        UserMessageFormatter::format_impl(self.localizer, err)
    }
}
...
impl UserMessageFormatter {
    ...
    fn format_impl<'a>(localizer: &dyn Localizer, err: &'a Error) -> Cow<'a, str> {
        match err {
            Error::WithSnippet { error, .. } => return Self::format_impl(localizer, error),
            _ => {}
        }
        ...
        match err {
            ...
            _ => DefaultMessageFormatter::format_impl(localizer, err),
        }
    }
}
