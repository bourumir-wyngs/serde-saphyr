...
fn scalar_conversion_error_carries_span_multiline() {
    ...
    let err = from_str::<bool>(r#"
definitely"#).expect_err("bool parse error expected");
    expect_location(&err, Location { row: 2, column: 1 });
    assert!(matches!(err, Error::Message { .. }));
}
...
fn unexpected_event_error_uses_event_location_multiline() {
    ...
    let err = from_str::<String>(r#"
- entry"#).expect_err("sequence cannot deserialize into string");
    expect_location(&err, Location { row: 2, column: 1 });
    assert!(matches!(err, Error::Unexpected { .. }));
}
...
fn parser_unknown_anchor_error_reports_location_multiline() {
    ...
    let err = from_str::<String>(r#"
*missing"#).expect_err("unknown anchor should error");
    expect_location(&err, Location { row: 2, column: 1 });
    assert!(matches!(err, Error::Message { .. }));
}
...
fn scalar_conversion_error_with_indent_reports_column() {
    ...
    expect_location(&err, Location { row: 1, column: 3 });
    assert!(matches!(err, Error::Message { .. }));
}
...
fn unexpected_sequence_with_indent_reports_column() {
    ...
    let err = from_str::<String>(r#"  - entry"#)
...
fn unexpected_mapping_on_second_line_with_indent() {
    ...
    let err = from_str::<String>(r#"
  k: 1"#).expect_err("mapping cannot deserialize into string");
    expect_location(&err, Location { row: 2, column: 3 });
    assert!(matches!(err, Error::Unexpected { .. }));
}
