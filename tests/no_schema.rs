...
fn no_schema_off_allows_plain_numeric_into_string() {
    ...
    opts.no_schema = false;
    let v: AsString = from_str_with_options(y, opts).expect("no_schema=false should accept plain 123 as string");
    assert_eq!(v.s, "123");
}
...
fn no_schema_on_accepts_quoted_numeric_into_string() {
    ...
    let mut opts = Options::default();
    opts.no_schema = true;
    let v: AsString = from_str_with_options(y, opts).expect("quoted numeric should be accepted as string when no_schema");
    assert_eq!(v.s, "123");
}
...
fn no_schema_on_accepts_explicit_str_tag_into_string() {
    ...
    let mut opts = Options::default();
    opts.no_schema = true;
    let v: AsString = from_str_with_options(y, opts).expect("!!str 123 should be accepted as string when no_schema");
    assert_eq!(v.s, "123");
}
...
fn no_schema_for_map_keys_string() {
    ...
    assert!(err.to_string().contains("must be quoted"), "unexpected: {err}");

    // Quoted numeric key should be accepted
    let y_quoted = "'1': a\n";
    let m = from_str_with_options::<BTreeMap<String, String>>(y_quoted, opts).expect("quoted key should pass");
...
fn no_schema_on_char_rejects_plain_numeric_or_bool_like() {
    ...
    assert!(e1.to_string().contains("must be quoted"), "unexpected: {e1}");
    ...
    assert!(e3.to_string().contains("must be quoted"), "unexpected: {e3}");
}

#[allow(dead_code)]
#[derive(Debug, Deserialize, PartialEq)]
enum UnitE {
    #[serde(rename = "true")] True,
    #[serde(rename = "1")] One,
    #[serde(rename = "ok")] Ok,
}
...
fn no_schema_for_enum_unit_variant_name() {
    ...
    off.no_schema = false;
    let e_true: UnitE = from_str_with_options("true\n", off.clone()).expect("true as unit variant should parse with no_schema=false");
    assert!(matches!(e_true, UnitE::True));
    let e_one: UnitE = from_str_with_options("1\n", off).expect("1 as unit variant should parse with no_schema=false");
    ...
    assert!(err_true.to_string().contains("must be quoted"), "unexpected: {err_true}");
    let err_one = from_str_with_options::<UnitE>("1\n", on.clone()).unwrap_err();
    assert!(err_one.to_string().contains("must be quoted"), "unexpected: {err_one}");

    // Quoted works with no_schema=true
    let e_true_q: UnitE = from_str_with_options("'true'\n", on.clone()).expect("quoted true should work");
...
#[derive(Debug, Deserialize, PartialEq)]
enum NewtypeE {
    #[serde(rename = "true")] True(i32),
    #[serde(rename = "1")] One(i32),
}
...
fn no_schema_for_enum_externally_tagged_map_form() {
    ...
    assert!(err1.to_string().contains("must be quoted"), "unexpected: {err1}");
    let err2 = from_str_with_options::<NewtypeE>("{ 1: 5 }\n", opts.clone()).unwrap_err();
    assert!(err2.to_string().contains("must be quoted"), "unexpected: {err2}");

    // Quoted keys should succeed
    let v1: NewtypeE = from_str_with_options("{ 'true': 7 }\n", opts.clone()).expect("quoted true key should work");
    assert!(matches!(v1, NewtypeE::True(7)));
    let v2: NewtypeE = from_str_with_options("{ '1': 9 }\n", opts).expect("quoted 1 key should work");
