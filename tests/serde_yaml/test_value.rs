...
use serde_saphyr::{Number, Value};

#[test]
fn test_nan() {
    let pos_nan = serde_saphyr::from_str::<Value>(".nan").unwrap();
    assert!(pos_nan.is_f64());
    assert_eq!(pos_nan, pos_nan);

    let neg_fake_nan = serde_saphyr::from_str::<Value>("-.nan").unwrap();
...
#[test]
fn test_digits() {
    let num_string = serde_saphyr::from_str::<Value>("01").unwrap();
