...
# Complete structure can only be build by merging multiple
# _anchors themselves that are not serialized into Rust structures
...
"#;
    ...
    let root: Root = serde_saphyr::from_str_with_options(yaml, Options {
...
"#;
    ...
    let entries: Vec<Entry> = serde_saphyr::from_str_with_options(yaml, Options {
        duplicate_keys: DuplicateKeyPolicy::FirstWins,
        .. Options::default()
    }).unwrap();
