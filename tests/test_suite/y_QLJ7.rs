...
// We attempt to parse the stream into Vec<BTreeMap<String, String>> using from_str_multiple and
...
fn yaml_qlj7_tag_shorthand_only_defined_in_first_doc_should_fail() {
    ...
    let r: Result<Vec<BTreeMap<String, String>>, _> = serde_saphyr::from_str_multiple(y);
    assert!(
        r.is_err(),
