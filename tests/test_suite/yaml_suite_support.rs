...
pub(super) fn assert_json_case(yaml: &str, expected_json: &str) {
    ...
    let actual = if expected.len() == 1 {
        ...
        serde_saphyr::from_str_multiple::<Value>(yaml)
...
pub(super) fn assert_invalid_case(yaml: &str) {
    ...
    match serde_saphyr::from_str_multiple::<IgnoredAny>(yaml) {
        Err(_) => {}
        Ok(documents) => panic!(
...
pub(super) fn assert_valid_events_with_options(
    ...
    serde_saphyr::from_str_multiple_with_options::<IgnoredAny>(yaml, options)
